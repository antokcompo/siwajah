-- ============================================================
-- 064: Fix Slot Deletion in absen_save_jadwal_slot RPC
--
-- Menjamin slot yang dihapus admin langsung dinonaktifkan (aktif = false)
-- dan dihapus dari database secara aman tanpa gagal foreign key.
--
...
BEGIN
  -- 1. Nonaktifkan / Hapus ID yang dihapus admin
  IF p_deleted_ids IS NOT NULL AND array_length(p_deleted_ids, 1) > 0 THEN
    FOREACH v_del_id IN ARRAY p_deleted_ids LOOP
      ...
      EXCEPTION WHEN foreign_key_violation THEN
        -- Tetap nonaktif (aktif = false) jika masih ada referensi riwayat scan
      END;
    END LOOP;
  END IF;
