-- ============================================================
-- 037: System Proteksi Anti-Fake GPS & Anti-Spoofing Scan Wajah
--
-- Perubahan:
--   - Tambah kolom is_mock_gps, gps_accuracy, fake_gps_score, fake_gps_reason pada absen_scan_wajah
--   - Update RPC absen_catat_scan_wajah untuk menyimpan indikator kecurangan GPS
--   - Update RPC absen_scan_wajah_harian untuk mengembalikan indikator Fake GPS
--
-- JALANKAN DI SUPABASE SQL EDITOR
-- ============================================================

-- 1. Tambah Kolom Anti-Fake GPS di tabel absen_scan_wajah
ALTER TABLE absen_scan_wajah ADD COLUMN IF NOT EXISTS is_mock_gps boolean DEFAULT false;
ALTER TABLE absen_scan_wajah ADD COLUMN IF NOT EXISTS gps_accuracy numeric DEFAULT NULL;
ALTER TABLE absen_scan_wajah ADD COLUMN IF NOT EXISTS fake_gps_score integer DEFAULT 0;
ALTER TABLE absen_scan_wajah ADD COLUMN IF NOT EXISTS fake_gps_reason text DEFAULT NULL;

-- 2. Update RPC: catat scan wajah — simpan data proteksi anti-fake GPS
...
-- 3. Update RPC: scan wajah harian — return indikator Anti-Fake GPS
...
RETURNS TABLE (
  ...
  SELECT
    ...
    sw.client_tz, sw.is_mock_gps, sw.gps_accuracy, sw.fake_gps_score, sw.fake_gps_reason
