-- ============================================================
-- FIX: Timezone handling in absen_import_dan_proses
--
-- Problem: timestamps are stored as UTC (timestamptz), but
-- config values (jam_masuk, jam_pulang, etc.) are in local time.
-- The old code compared UTC times against local config values,
-- causing wrong time classification.
--
-- Fix: Read zona_waktu from absen_konfigurasi and use
-- AT TIME ZONE to convert to local time before comparing.
-- Supports WIB (Asia/Jakarta), WITA (Asia/Makassar), WIT (Asia/Jayapura).
-- ============================================================

-- Add timezone config key if not exists
INSERT INTO absen_konfigurasi (key, value, deskripsi)
VALUES ('zona_waktu', 'Asia/Jayapura', 'Zona waktu lokasi proyek (Asia/Jakarta=WIB, Asia/Makassar=WITA, Asia/Jayapura=WIT)')
...
BEGIN
  ...
  -- KEY FIX: use AT TIME ZONE to convert UTC -> local time before extracting time/date
  FOR v_rec IN
    SELECT
      karyawan_id,
