-- ============================================================
-- 050: Fitur Denda / Potongan Gaji & Preservation in absen_hitung_gaji
--
-- Perubahan:
--   - Memastikan kolom potongan numeric DEFAULT 0 ada di tabel absen_gaji_bulanan.
--   - Update RPC absen_hitung_gaji agar saat Hitung Gaji dijalankan ulang,
--     potongan/denda yang telah diinput Admin TIDAK terhapus/nol, dan total_gaji
--     dikalkulasi secara presisi: Gaji Pokok + Gaji Lembur + Tunjangan - Potongan.
--
-- JALANKAN DI SUPABASE SQL EDITOR
-- ============================================================

ALTER TABLE absen_gaji_bulanan ADD COLUMN IF NOT EXISTS potongan NUMERIC DEFAULT 0;
...
BEGIN
  ...
  FOR v_rec IN
    ...
    emp_aggregates AS (
      SELECT
        ...
        COALESCE((
          SELECT potongan FROM absen_gaji_bulanan 
          WHERE karyawan_id = k.id AND bulan = p_bulan AND tahun = p_tahun
        ), 0) AS existing_potongan
      FROM absen_karyawan k
      WHERE k.status_aktif = true
        AND (p_kode_proyek IS NULL OR k.kode_proyek = p_kode_proyek)
    )
  ...
  LOOP
    ...
    -- Flat Overtime Formula: Total Jam Lembur Approved * Upah Per Jam (Tanpa 1.5x / 2.0x)
    IF v_rec.total_lembur > 0 THEN
      v_gaji_lembur := v_rec.total_lembur * v_upah_lembur_perjam;
    ELSE
      v_gaji_lembur := 0;
    END IF;
    ...
    VALUES (
      ...
      v_rec.tunjangan,
      v_rec.existing_potongan,
      v_gaji_pokok + ROUND(v_gaji_lembur, -2) + v_rec.tunjangan - v_rec.existing_potongan,
      'draft',
      v_is_full
    )
    ON CONFLICT (karyawan_id, bulan, tahun) DO UPDATE SET
      ...
      total_gaji = EXCLUDED.gaji_pokok + EXCLUDED.gaji_lembur + EXCLUDED.tunjangan - COALESCE(absen_gaji_bulanan.potongan, 0),
